 ...
 */
class AsyncFilterOperatorSpec<M> extends AsyncStreamOperatorSpec<M, M> {
  private final AsyncFilterFunction<M> filterFn;

  AsyncFilterOperatorSpec(AsyncFilterFunction<M> filterFn, String opId) {
    super(new AsyncFlatMapFunction<M, M>() {
      @Override
      public CompletableFuture<Collection<M>> apply(M message) {
        return filterFn.apply(message)
            .thenApply(shouldInclude -> {
                if (shouldInclude) {
